    ...
    def test_compute_transition_matrix(self):
        ...
        npt.assert_almost_equal(
            ...
            np.linalg.inv(np.block(
                [[-np.identity(2)/2, np.zeros((2, 2)), np.zeros((2, 2)),
                  np.zeros((2, 2))],
                 [np.identity(2)/2, -np.identity(2)/2, np.zeros((2, 2)),
                  np.zeros((2, 2))],
                 [np.zeros((2, 2)), np.identity(2)/2, -np.identity(2)/2,
                  np.zeros((2, 2))],
                 [np.zeros((2, 2)), np.zeros((2, 2)), np.identity(2)/2,
                  -np.identity(2)/2]]))
        )
