...
class TestWarwickICs(unittest.TestCase):
    ...
    def test__init__(self):
        ...
        ICs = em.WarwickICs(
            model=model,
            susceptibles_IC=susceptibles,
            exposed_f_IC=exposed_f,
            exposed_sd_IC=exposed_sd,
            exposed_su_IC=exposed_su,
            exposed_q_IC=exposed_q,
        ...
        npt.assert_array_equal(np.array(ICs.exposed_f),
                               np.array([[0, 0], [0, 0]]))

        npt.assert_array_equal(np.array(ICs.exposed_sd),
                               np.array([[0, 0], [0, 0]]))

        npt.assert_array_equal(np.array(ICs.exposed_su),
                               np.array([[0, 0], [0, 0]]))

        npt.assert_array_equal(np.array(ICs.exposed_q),
                               np.array([[0, 0], [0, 0]]))
    ...
    def test__call__(self):
        ...
        self.assertEqual(
            ICs(),
            [susceptibles, exposed_f, exposed_sd, exposed_su,
